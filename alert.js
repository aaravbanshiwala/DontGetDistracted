...
function setupAlertListeners() {
  ...
  if (backBtn) {
    backBtn.addEventListener("click", () => {
      ...
      // User acknowledges and gets back to work: clear counters and snooze
  ...
  if (dismissBtn) {
    dismissBtn.addEventListener("click", () => {
      ...
      // User dismisses: snooze future alerts
      chrome.runtime.sendMessage({ type: "alert_dismissed" }, (response) => {
        if (chrome.runtime.lastError) {
          console.error("Error sending message:", chrome.runtime.lastError);
        }
        console.log("[DGD] Dismissed, closing tab");
        window.close();
      });
    });
  }
